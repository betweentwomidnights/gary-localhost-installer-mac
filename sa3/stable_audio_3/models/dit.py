...
from .lora import has_lora, enable_lora, disable_lora, filter_lora_layers
...
class DiffusionTransformer(nn.Module):
    ...
    def apg_project(self, v0, v1, padding_mask=None):
        ...
        """
        dtype = v0.dtype
        # MPS does not support float64 tensors. Keep the original higher-precision
        # CPU/CUDA path, but use float32 on MPS so Gradio's APG control works there.
        project_dtype = torch.float32 if v0.device.type == "mps" else torch.float64
        v0, v1 = v0.to(project_dtype), v1.to(project_dtype)

        if padding_mask is not None:
            # Expand mask to match tensor shape: (B, T) -> (B, 1, T)
            mask = padding_mask.unsqueeze(1).to(project_dtype)
