...
class AceStepConditionGenerationModel(AceStepPreTrainedModel):
    ...
    def prepare_condition(
        ...
        # tokenize/detokenize LM hints only when cover conditioning is active.
        # This avoids unnecessary LM hint computation (and torch.where branch interactions)
        # for pure lego/text2music paths where is_covers is all-zero.
        has_cover = is_covers is not None and bool((is_covers > 0).any().item())
        if has_cover:
