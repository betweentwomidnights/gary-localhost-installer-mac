...
class LLMHandler:
    ...
    def initialize(
        ...
        try:
            ...
            # Also disable on CUDA: the SDPA-based paged attention kernel uses .item()
            # during capture, which is a CPU-GPU sync forbidden inside a CUDA graph.
            # The capture fails, leaving the default stream's RNG in a contaminated state
            # that subsequently breaks the sampler's exponential_() call at inference time.
            # Eager mode has no functional impact — decode is fast enough without graphs.
            enforce_eager_for_vllm = bool(is_rocm) or torch.cuda.is_available()
