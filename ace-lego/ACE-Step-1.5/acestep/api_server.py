...
def _get_checkpoint_dir(project_root: Optional[str] = None) -> str:
    """Resolve the checkpoint directory, honoring ACESTEP_CHECKPOINT_DIR when set."""
    configured = os.getenv("ACESTEP_CHECKPOINT_DIR", "").strip()
    if configured:
        expanded = os.path.expanduser(configured)
        if os.path.isabs(expanded):
            checkpoint_dir = os.path.realpath(expanded)
        else:
            root = project_root or _get_project_root()
            checkpoint_dir = os.path.realpath(os.path.join(root, expanded))
    else:
        root = project_root or _get_project_root()
...
class GenerateMusicRequest(BaseModel):
    ...
    task_type: str = Field(
        default="text2music",
        description="Task type: text2music, cover, cover-nofsq, repaint, extract, lego, complete",
    )
...
def create_app() -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        ...
        async def _run_one_job(job_id: str, req: GenerateMusicRequest) -> None:
            ...
            def _blocking_generate() -> Dict[str, Any]:
                ...
                use_cot_metas = bool(req.use_cot_metas)
                ...
                # - use_cot_caption/use_cot_language/use_cot_metas (LM enhances metadata)
                want_llm = use_cot_caption or use_cot_language or use_cot_metas
                ...
                if want_llm and not llm_available:
                    if use_cot_caption or use_cot_language or use_cot_metas:
                        print(
                            ...
                            f"use_cot_metas={use_cot_metas}->False"
                        )
                ...
                # Respect client flag while preserving existing sample/format gating.
                effective_use_cot_metas = use_cot_metas and (not sample_mode) and (not format_has_duration)
                ...
                params = GenerationParams(
                    ...
                    # use_cot_metas logic:
                    # - still gated off for sample_mode and format_has_duration
                    # - can now be explicitly disabled by clients
        ...
        if no_init:
            ...
            # Keep --no-init behavior consistent with normal startup LLM policy.
            # When LLM is disabled by env/GPU policy, block lazy LM downloads.
            init_llm_env = os.getenv("ACESTEP_INIT_LLM", "").strip().lower()
            llm_lazy_allowed = gpu_config.init_lm_default
            if init_llm_env in {"", "auto"}:
                pass
            elif init_llm_env in {"1", "true", "yes", "y", "on"}:
                llm_lazy_allowed = True
            else:
                llm_lazy_allowed = False
            ...
            if llm_lazy_allowed:
                ...
                app.state._llm_init_error = (
                    "LLM disabled by ACESTEP_INIT_LLM/GPU policy. "
                    "Enable ACESTEP_INIT_LLM=true to use thinking/sample/format features."
                )
                print("[API Server] LLM lazy loading disabled in --no-init mode")
    ...
    async def create_music_generate_job(request: Request, authorization: Optional[str] = Header(None)):
        ...
        def _build_request(p: RequestParser, **kwargs) -> GenerateMusicRequest:
            ...
            return GenerateMusicRequest(
                ...
                use_cot_metas=p.bool("use_cot_metas", True),
    ...
    async def load_model(
        ...
        if getattr(app.state, "_soft_unloaded", False) and not config_path:
            has_warm_state = (
                ...
                and handler.text_encoder is not None
                and handler.text_tokenizer is not None
        ...
        if params is None:
            ...
            params = {
                "project_root": project_root,
                "config_path": os.getenv("ACESTEP_CONFIG_PATH", "acestep-v15-base"),
                "device": os.getenv("ACESTEP_DEVICE", "auto"),
                "use_flash_attention": _env_bool("ACESTEP_USE_FLASH_ATTENTION", True),
                "compile_model": _env_bool("ACESTEP_COMPILE_MODEL", False),
        ...
        status_msg, ok = handler.initialize_service(**{
            ...
            if k in ("project_root", "config_path", "device", "use_flash_attention",
                     "compile_model", "offload_to_cpu", "offload_dit_to_cpu", "quantization")
        })
        if not ok:
    ...
    async def unload_model(
        mode: Optional[str] = Query(None, description="Unload mode: soft or hard"),
        _: None = Depends(verify_api_key),
    ):
        ...
        import gc
        handler: AceStepHandler = app.state.handler
        requested_mode = (mode or os.getenv("ACESTEP_UNLOAD_STRATEGY", "hard")).strip().lower()
        unload_mode = "soft" if requested_mode in {"soft", "logical", "warm"} else "hard"
        try:
            if unload_mode == "soft":
                # Keep model objects resident for fast subsequent /v1/load calls.
                app.state._initialized = False
                app.state._soft_unloaded = True
                return _wrap_response({"status": "unloaded_soft", "mode": unload_mode})
            ...
            for attr in (
                ...
                "tokenize",
                "detokenize",
                # MLX-side objects / compiled callables
                "mlx_decoder",
                "mlx_vae",
                "_mlx_compiled_decode",
                "_mlx_compiled_encode_sample",
                "_mlx_compiled_encode_mean",
                "_mlx_vae_dtype",
            ):
            ...
            # Keep MLX backend flags in a known state after hard unload.
            if hasattr(handler, "use_mlx_dit"):
                handler.use_mlx_dit = False
            if hasattr(handler, "use_mlx_vae"):
                handler.use_mlx_vae = False
            ...
            app.state._initialized = False
            app.state._soft_unloaded = False
            return _wrap_response({"status": "unloaded", "mode": unload_mode})
        except Exception as e:
