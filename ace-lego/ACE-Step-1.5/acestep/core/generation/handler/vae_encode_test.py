...
class _DistWithMode(_Dist):
    """Distribution stub exposing both ``sample`` and ``mode``."""

    def __init__(self, sample, mode):
        """Store sample/mode tensors."""
        super().__init__(sample)
        self._mode = mode

    def mode(self):
        """Return deterministic latent mode."""
        return self._mode
...
class VaeEncodeMixinTests(unittest.TestCase):
    ...
    def test_vae_latent_from_dist_prefers_mode_by_default(self):
        """Deterministic mode should be selected when sampling is not requested."""
        host = _Host()
        sample = torch.full((1, 4, 2), 11.0)
        mode = torch.full((1, 4, 2), 22.0)
        dist = _DistWithMode(sample=sample, mode=mode)

        with patch.dict(os.environ, {"ACESTEP_VAE_ENCODE_SAMPLE": "0"}, clear=False):
            chosen = host._vae_latent_from_dist(dist)
        self.assertTrue(torch.equal(chosen, mode))

    def test_vae_latent_from_dist_can_force_sampling(self):
        """Sampling should be used when ACESTEP_VAE_ENCODE_SAMPLE=1."""
        host = _Host()
        sample = torch.full((1, 4, 2), 11.0)
        mode = torch.full((1, 4, 2), 22.0)
        dist = _DistWithMode(sample=sample, mode=mode)

        with patch.dict(os.environ, {"ACESTEP_VAE_ENCODE_SAMPLE": "1"}, clear=False):
            chosen = host._vae_latent_from_dist(dist)
        self.assertTrue(torch.equal(chosen, sample))
