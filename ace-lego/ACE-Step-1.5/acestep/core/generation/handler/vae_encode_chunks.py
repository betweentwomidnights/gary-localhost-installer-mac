...
class VaeEncodeChunksMixin:
    ...
    def _resolve_chunk_latent(self, latent_dist):
        """Resolve latent tensor from distribution with deterministic default."""
        if hasattr(self, "_vae_latent_from_dist"):
            return self._vae_latent_from_dist(latent_dist)
        if hasattr(latent_dist, "mode"):
            return latent_dist.mode()
        latent_mean = getattr(latent_dist, "mean", None)
    ...
    def _tiled_encode_gpu(self, audio, batch_size, samples, stride, overlap, num_steps, chunk_size):
        ...
        for i in tqdm(range(num_steps), desc="Encoding audio chunks", disable=self.disable_tqdm):
            ...
            with torch.inference_mode():
                latent_dist = self.vae.encode(audio_chunk).latent_dist
                latent_chunk = self._resolve_chunk_latent(latent_dist)
    ...
    def _tiled_encode_offload_cpu(self, audio, batch_size, samples, stride, overlap, num_steps, chunk_size):
        ...
        with torch.inference_mode():
            first_latent_dist = self.vae.encode(first_audio_chunk).latent_dist
            first_latent_chunk = self._resolve_chunk_latent(first_latent_dist)
