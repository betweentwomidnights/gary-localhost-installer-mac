...
class InitServiceSetupMixin:
    ...
    def _initialize_mlx_backends(
        ...
        *,
        ...
        env_use_mlx_dit = os.environ.get("ACESTEP_USE_MLX_DIT")
        if env_use_mlx_dit is not None:
            use_mlx_dit = env_use_mlx_dit.strip().lower() in {"1", "true", "yes", "y", "on"}
            logger.info(
                f"[initialize_service] ACESTEP_USE_MLX_DIT={env_use_mlx_dit!r} -> use_mlx_dit={use_mlx_dit}"
            )

        use_mlx_vae = True
        env_use_mlx_vae = os.environ.get("ACESTEP_USE_MLX_VAE")
        if env_use_mlx_vae is not None:
            use_mlx_vae = env_use_mlx_vae.strip().lower() in {"1", "true", "yes", "y", "on"}
            logger.info(
                f"[initialize_service] ACESTEP_USE_MLX_VAE={env_use_mlx_vae!r} -> use_mlx_vae={use_mlx_vae}"
            )
