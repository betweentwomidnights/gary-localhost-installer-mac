...
class InitServiceCatalogMixin:
    ...
    def _resolve_checkpoint_dir(self, project_root: Optional[str] = None) -> str:
        """Resolve checkpoint directory, honoring ACESTEP_CHECKPOINT_DIR when set."""
        ...
        base_root = project_root or self._get_project_root()
        return os.path.normpath(os.path.join(base_root, "checkpoints"))

    def get_available_checkpoints(self) -> List[str]:
        """Return available checkpoint directory paths under the project root."""
        checkpoint_dir = self._resolve_checkpoint_dir(self._get_project_root())
