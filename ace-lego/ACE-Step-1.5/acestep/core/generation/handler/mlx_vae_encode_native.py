...
class MlxVaeEncodeNativeMixin:
    ...
    def _should_sample_mlx_vae_latent(self) -> bool:
        """Return True when stochastic MLX VAE latent sampling is enabled."""
        sample_flag = os.environ.get("ACESTEP_MLX_VAE_ENCODE_SAMPLE", "").strip().lower()
        if sample_flag:
            return sample_flag in ("1", "true", "yes", "on")
        return os.environ.get("ACESTEP_VAE_ENCODE_SAMPLE", "0").strip().lower() in (
            "1",
            "true",
            "yes",
            "on",
    ...
    def _resolve_mlx_encode_fn(self):
        ...
        """
        if not self._should_sample_mlx_vae_latent():
            encode_fn = getattr(self, "_mlx_compiled_encode_mean", None)
            if encode_fn is not None:
                return encode_fn
            if self.mlx_vae is not None and hasattr(self.mlx_vae, "encode_mean"):
                return self.mlx_vae.encode_mean
