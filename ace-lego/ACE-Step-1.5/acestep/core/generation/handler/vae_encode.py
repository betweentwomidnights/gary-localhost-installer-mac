...
class VaeEncodeMixin:
    ...
    def _should_sample_vae_latent(self) -> bool:
        """Return True when stochastic VAE encoding is explicitly enabled."""
    ...
    def _vae_latent_from_dist(self, latent_dist):
        """Select deterministic latent (mode/mean) unless sampling is requested."""
        if self._should_sample_vae_latent():
    ...
    def tiled_encode(self, audio, chunk_size=None, overlap=None, offload_latent_to_cpu=True):
        ...
        if samples <= chunk_size:
            ...
            with torch.inference_mode():
                ...
                latents = self._vae_latent_from_dist(latent_dist)
            if input_was_2d:
