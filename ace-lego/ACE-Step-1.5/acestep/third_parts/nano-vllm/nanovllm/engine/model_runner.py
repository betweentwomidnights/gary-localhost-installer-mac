...
class ModelRunner:
    ...
    def __init__(self, config: Config, rank: int, event: Event | list[Event]):
        ...
        if not self.enforce_eager:
            # Save CUDA RNG state before capture attempt so we can fully restore it
            # if capture fails. torch.cuda.synchronize() alone does not reset the
            # generator's internal graphStatus_ flag, leaving it stuck in capture mode
            # and causing "Offset increment outside graph capture" errors during inference.
            _rng_state = torch.cuda.get_rng_state()
            try:
                self.capture_cudagraph()
            except Exception as e:
                torch.cuda.set_rng_state(_rng_state)
                torch.cuda.synchronize()
                self.graphs = {}
                logger.warning(
                    f"[nanovllm] CUDA graph capture failed ({type(e).__name__}); "
                    "running in eager mode (no functional impact)."
                )
