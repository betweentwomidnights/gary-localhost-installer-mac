...
cmd_generate() {
    ...
    local model="" language="" steps="" guidance="" seed="" duration="" bpm="" key="" batch=""
    local task_type="text2music" src_audio="" track_name=""
    ...
    [ -n "$key" ] && payload=$(echo "$payload" | jq --arg v "$key" '. + {key_scale: $v}')
    [ -n "$batch" ] && payload=$(echo "$payload" | jq --argjson v "$batch" '. + {batch_size: $v}')
    [ -n "$task_type" ] && [ "$task_type" != "text2music" ] && payload=$(echo "$payload" | jq --arg v "$task_type" '. + {task_type: $v}')
    [ -n "$track_name" ] && payload=$(echo "$payload" | jq --arg v "$track_name" '. + {track_name: $v}')

    local api_mode=$(load_api_mode)

    echo "Generating music..."
    if [ -n "$src_audio" ]; then
        echo "  Mode: ${task_type^^} (src: $(basename "$src_audio"))"
        [ -n "$track_name" ] && echo "  Track: $track_name"
    ...
    if [ "$api_mode" = "completion" ]; then
        ...
        [ -n "$key" ] && payload_c=$(echo "$payload_c" | jq --arg v "$key" '.audio_config.key_scale = $v')
        ...
        if [ -n "$src_audio" ]; then
            # Multipart form upload — required for lego/repaint/cover (absolute paths blocked in JSON)
            # Extract all JSON fields as individual -F form args
            local form_args=()
            while IFS='=' read -r k v; do
                form_args+=(-F "${k}=${v}")
            done < <(echo "$payload" | jq -r 'to_entries[] | "\(.key)=\(.value)"')
            form_args+=(-F "ctx_audio=@${src_audio}")
