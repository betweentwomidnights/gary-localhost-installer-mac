...
def generate_diffusion_cond(
        ...
        seed: int = -1,
        device: tp.Union[str, torch.device] = "cuda",
        init_audio: tp.Optional[tp.Tuple[int, torch.Tensor]] = None,
        init_noise_level: float = 1.0,
        return_latents = False,
        **sampler_kwargs
    ...
    # Helper: only touch CUDA backends on CUDA
    def _is_cuda(d: torch.device) -> bool:
        return d.type == "cuda"

    # Helper: autocast context by device
    # - CUDA: torch.autocast("cuda", dtype=torch.float16)
    # - MPS: torch.autocast("mps", dtype=torch.float16) (float16 only on MPS)
    # - CPU: no autocast by default
    def _autocast_ctx(d: torch.device):
        if d.type == "cuda":
    ...
    torch.manual_seed(seed)
    # Define the initial noise immediately after setting the seed
    noise = torch.randn([batch_size, model.io_channels, sample_size], device=dev)
    ...
    if conditioning_tensors is None:
        conditioning_tensors = model.conditioner(conditioning, dev)
    conditioning_inputs = model.get_conditioning_inputs(conditioning_tensors)

    if negative_conditioning is not None or negative_conditioning_tensors is not None:
        
        if negative_conditioning_tensors is None:
            negative_conditioning_tensors = model.conditioner(negative_conditioning, dev)
    ...
    if init_audio is not None:
        ...
        # Prepare the initial audio for use by the model
        init_audio = prepare_audio(init_audio, in_sr=in_sr, target_sr=model.sample_rate, target_length=audio_sample_size, target_channels=io_channels, device=dev)
    ...
    if diff_objective == "v":    
        ...
        with _autocast_ctx(dev):
            sampled = sample_k(model.model, noise, init_audio, steps, **sampler_kwargs, **conditioning_inputs, **negative_conditioning_tensors, cfg_scale=cfg_scale, batch_cfg=True, rescale_cfg=True, device=dev)
    elif diff_objective in ["rectified_flow", "rf_denoiser"]:
        ...
        with _autocast_ctx(dev):
            sampled = sample_rf(model.model, noise, init_data=init_audio, steps=steps, **sampler_kwargs, **conditioning_inputs, **negative_conditioning_tensors, dist_shift=model.dist_shift, cfg_scale=cfg_scale, batch_cfg=True, rescale_cfg=True, device=dev)
