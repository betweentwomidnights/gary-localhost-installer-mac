...
def _autocast_ctx(device: tp.Union[torch.device, str], enabled: bool = True, dtype: torch.dtype = torch.float16):
    if not enabled:
...
class CLAPAudioConditioner(Conditioner):
    ...
    def forward(self, audios: tp.Union[torch.Tensor, tp.List[torch.Tensor], tp.Tuple[torch.Tensor]] , device: tp.Any = "cuda") -> tp.Any:
        ...
        with _autocast_ctx(device, enabled=False):
...
class T5Conditioner(Conditioner):
    ...
    def forward(self, texts: tp.List[str], device: tp.Union[torch.device, str]) -> tp.Tuple[torch.Tensor, torch.Tensor]:
        ...
        with _autocast_ctx(device, dtype=torch.float16), torch.set_grad_enabled(self.enable_grad):
